...
/**
 * Command line application for converting cave survey data from one file format
 * to another.
 * Currently it can read some survex files, text export files from PocketTopo
 * and DXF format
...
public class CaveConverter {
	...
	/**
	 * Supported cave survey data formats for input (these are the formats the
	...
	/**
	 * Supported cave survey data formats for output (these are the formats the
	...
	private static String dataFormats[][] = { { "Compass", "dat" }, { "DXF", "dxf" }, { "PocketTopo", "txt" },
			{ "Survex", "svx" }, { "Toporobot", "text" } };
	 ...
	 * @return The survey data input format matching the file extn (or null if no
	 *         match)
	 */
		...
		if (args.length > 0) {
			...
			if (args.length > 1) {
				...
				if (args.length > 2) {
					...
					try {
						inputFormat = UtilityFunctions
								.inputDataFormatFromLetterCode(parseSingleCharacterArgument(args[2]));
						if (args.length > 3) {
							// Set output format from 4th argument
							outputFormat = UtilityFunctions
									.outputDataFormatFromLetterCode(parseSingleCharacterArgument(args[3]));
	 ...
	 * @throws ParseException If the argument is not a single character length
	 *                        string
	 */
	...
	/**
	 * Reads in a survey data file and converts it to another format which is
	 ...
	 * @param splaysOpt       Indicates if a splays output option was set to True,
	 *                        False or to use default for writer (T,F,D)
	 * @param generateLRUDOpt Indicates whether to generate LRUD data from splays
	 *                        before writing out data
	 * @param charSetEncoding Character set encoding to be used when reading and
	 *                        writing files
	 * @param logger          Logging class to output information, warning and error
	 *                        messages to
	...
	public static void convertFile(String inputFilename, String outputFilename, SurveyDataInputFormats inputFormat,
		...
		if (surveyData != null) {
			...
			// }
				...
				logger.logMessage(
						"Unsupported output format argument: " + outputFormat + " is not a valid output format.");
			}
		}
	...
	/**
	 * Generates a string from a number with specified number of decimal places
	 * using the English decimal
	...
	/**
	 * Generates a string from a number using the English decimal separator (.) and
	 * rounded down
	 * to 2 decimal places if longer. Numbers with less decimal places are not
	 * padded with zeros.
	 * 
	 * @param num Number to convert to string
	 * @return Formatted string representation of number
	...
	/**
	 * Prefixes a string with space characters to make the string a minimum length
	 * if
	 ...
	 * @return The string padded with space characters to make it at least the
	 *         specified length
	 */
