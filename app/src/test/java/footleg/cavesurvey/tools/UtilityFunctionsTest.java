...
public class UtilityFunctionsTest {
	...
	/**
	 * Test helper method to call the UtilityFunctions.averageCompassBearings method
	 * for
	 * two given bearings
	 */
	 ...
	 * Test helper method to call the UtilityFunctions.averageCompassBearings method
	 * for
	 * two given bearings and an expected result, which formats a useful assertion
	 * message
	 * on test failure.
	 */
	private void testMean2(double a, double b, double expectedMean) {
		double mean = calcMeanBearing(a, b);
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for values of " + a + " and " + b + ".");
	}

	/**
	 * Test helper method to call the UtilityFunctions.averageCompassBearings method
	 * for
	 * three given bearings and an expected result, which formats a useful assertion
	 * message
	 * on test failure.
	 */
	private void testMean3(double a, double b, double c, double expected) {
		...
		assertEquals(expectedMean, mean, meanBearingTolerance,
				"Mean bearing for values of " + a + ", " + b + ", and " + c + ".");
	}

	/**
	 * Test helper method to compare the result from
	 * UtilityFunctions.averageCompassBearings method
	...
	/**
	 * Test method which calculates the numerical average of two bearings using
	 * logical cases
	 * for relative positions of the two legs. This method essentially adds the two
	 * bearings
	 * together and divides by two to get an average but also handles the case where
	 * the bearings
	 * are either side of North (i.e. 359 & 1 should average to 0 and not 180).
	 * This method works for any pair of bearings in the range 0 - 360 degrees with
	 * any
	 * angle between them.
	 */
	private double numericalMeanBearing2(double bearing1, double bearing2) {
		...
		if (bearing1 == bearing2) {
			...
			if (adjustedBearing > 180) {
				...
				meanBearing = UtilityFunctions
						.adjustBearingWithinDegreesRange(360 - ((360 - adjustedBearing) / 2) + smallerBearing, 0, 360);
			} else {
	...
	/**
	 * Test method which calculates the numerical average of three bearings using
	 * logical cases
	 * for relative positions of the three legs when all within 180 segment of each
	 * other. This
	 * method essentially adds the three bearings together and divides by three to
	 * get an average
	 * but also handles the case where the bearings are either side of North (i.e.
	 * 359, 357, 1
	...
	private double numericalMeanBearing3(double bearing1, double bearing2, double bearing3) {
		...
		if ((bearingA == bearingB) && (bearingA == bearingC)) {
			...
			// Check for case where all bearings are in the northern half of the compass
			// dial
			...
			if (((bearingA > 270) || (bearingA < 90))
				...
				if ((bearingA < 180) || (bearingB < 180) || (bearingC < 180)) {
					// At least one bearing on East side of dial, make any West side bearings
					// negative
					if (bearingA > 180) {
						bearingA -= 360;
					}
	...
	/**
	 * Test method for
	 * {@link footleg.cavesurvey.tools.UtilityFunctions#averageCompassBearings(double[] bearings)}.
	 */
	@Test
	public void testMeanBearings() {
		// Test average trend of a pair of bearings using vector averaging (atan2
	...
	public void testMeanCompassBearing() {
		...
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for 2 similar values just over zero.");

		expectedMean = 180.0;
		mean = calcMeanBearing(179, 181);
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for 2 similar values around 180 deg.");

		expectedMean = 180.0;
		mean = calcMeanBearing(90, 270);
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for 2 values exactly East and West");

		expectedMean = 180.0;
		mean = calcMeanBearing(270, 90);
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for 2 values exactly West and East");

		expectedMean = 0.0;
		mean = calcMeanBearing(271, 89);
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for 2 values just North of West and East");

		expectedMean = 320.0;
		mean = calcMeanBearing(20, 260);
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for 2 values over 180 degrees different");
	...
	public void testMeanBearings3() {
		...
		// Default increments for fast testing (test takes approx. 1 sec. with these
		// values)
		double mainBearingIncrement = 7.3; // Bearing 1 tested from 0-360 deg in steps of this size
		double bearingSpreadIncrement = 0.3; // Maximum difference between all 3 bearings varies from 0-5 deg in steps
												// of this size
		double bearingSpreadDiffIncrement = 0.07; // Difference between bearing 1 and 2 varies in steps of this size
													// between min and max.

		if (rigourousTests) {
			// Use smaller increments for better test coverage of numeric space at cost of
			// running time
	...
	public void testAverageCompassBearings() {
		...
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for 3 similar values just over zero.");

		expectedMean = 180.0;
		mean = calcMeanBearing(179, 180, 181);
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for 3 similar values around 180 deg.");

		expectedMean = 359.5;
		mean = calcMeanBearing(359.9, 359.2, 359.4);
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for 3 similar values around 359.5 deg.");

		expectedMean = 0.09;
		mean = calcMeanBearing(359.94, 0.21, 0.12);
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for 3 values either side of North");

		expectedMean = 359.9966667;
		mean = calcMeanBearing(359.94, 0.03, 0.02);
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for 3 values either side of North");

		// These bearings spread over 10 degree range show difference between mean
		// vector angle and
		// straight numerical averaging methods of calculating. So the correct answer is
		// not 2.000
		expectedMean = 2.00244;
		mean = calcMeanBearing(356, 4, 6);
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for 3 values around 0");
		...
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for 3 values around 45");
		...
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for 3 values around 90");
		...
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for 3 values around 135");
		...
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for 3 values around 180");

		expectedMean = 223.002;
		mean = calcMeanBearing(225, 227, 217);
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for 3 values around 225");

		expectedMean = 268.002;
		mean = calcMeanBearing(270, 272, 262);
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for 3 values around 270");

		expectedMean = 313.002;
		mean = calcMeanBearing(315, 317, 307);
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for 3 values around 315");

		expectedMean = 357.002;
		mean = calcMeanBearing(359, 1, 351);
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for 3 values around 360");

		expectedMean = 4.610;
		mean = calcMeanBearing(0.0, 0.0, 13.86);
		assertEquals(expectedMean, mean, meanBearingTolerance, "Mean bearing for 3 values around 360");

		// These bearings spread over 22.5 degree range show difference between mean
		// vector angle and
		// straight numerical averaging methods of calculating. So the correct answer is
		// not 116.533
		...
		// These bearings spread over 160 degree range show difference between mean
		// vector angle and
		// straight numerical averaging methods of calculating. So the correct answer is
		// not 126.667
		// as clearly with bearings pulling almost North or South the average trend
		// would be in the
		// North-South direction, and pulling Eastwards nearly as much as a numerical
		// average suggests.
	...
	/**
	 * Test method for
	 * {@link footleg.cavesurvey.tools.UtilityFunctions#bearingDifferenceDegrees(double angle1, double angle2)}.
	 */
	@Test
	public void testBearingDifference() {
		...
		assertEquals(expectedDiff, calcDiff, meanBearingTolerance,
				"Difference between two bearings either side of North");

		expectedDiff = 180.0;
		calcDiff = UtilityFunctions.bearingDifferenceDegrees(90.0, 270.0);
		assertEquals(expectedDiff, calcDiff, meanBearingTolerance,
				"Difference between two bearings of exactly East and West");

		expectedDiff = 4.8;
		calcDiff = UtilityFunctions.bearingDifferenceDegrees(120.4, 125.2);
		assertEquals(expectedDiff, calcDiff, meanBearingTolerance,
				"Difference between two bearings of similar direction");
	}

	/**
	 * Test method for
	 * {@link footleg.cavesurvey.tools.UtilityFunctions#stringToDate( String dateString, String dateFormat )},
	...
	/**
	 * Converts a string from one data format to another, with parse exception
	 * handling.
	...
	private void doDateStringConversion(String dateIn, String dateInFormat,
		...
		try {
			...
			assertEquals(dateExpectedOut, dateOut, "Date converted from one string format to another");
	...
	/**
	 * Test method for
	 * {@link footleg.cavesurvey.tools.UtilityFunctions#parseDataStringIntoDataItems( String dataIn )}.
	 */
	@Test
	public void testParseDataLineIntoDataItems() {
		...
		dataLine = dataItems[0] + " " + dataItems[1] + "  " + dataItems[2] + "\t\"" + dataItems[3] + "\" \t\""
				+ dataItems[4] + "\"\t " + dataItems[5];
	...
	private void splitStringIntoItemsTest(String dataLine, String[] items) {
		...
		assertEquals(items.length, parsedData.size(),
				"Data string '" + dataLine + "' returns expected number of items.");

		// Check each item returned matches expected data item
		for (int i = 0; i < items.length; i++) {
			assertEquals(items[i], parsedData.get(i), "Item " + (i + 1) + " in '" + dataLine + "'.");
		}
	}

	/**
	 * Test method for
	 * {@link footleg.cavesurvey.tools.UtilityFunctions#convertToLinearSeries( SurveySeries seriesIn )}.
	 */
	@Test
	public void testConvertToLinearSeriesOrderedT() {
		...
		assertEquals(1, convertedSeries.getLinks().size(), "Expect links between all series.");
		SeriesLink firstLink = convertedSeries.getLinks().get(0);
		assertEquals(convertedSeries.getInnerSeries(0).getSeriesName(), firstLink.getSeries2(),
				"Expect link from series 1.");
		assertEquals(convertedSeries.getInnerSeries(1).getSeriesName(), firstLink.getSeries1(),
				"Expect link to series 2.");

	}

	/**
	 * Test method for
	 * {@link footleg.cavesurvey.tools.UtilityFunctions#compareStringLists(
	...
	public void testCompareStringLists() {
		...
		assertTrue(UtilityFunctions.compareStringLists(list1, list2),
				"Expect pair of empty string lists to be considered identical.");

		list1.add("A String");

		// Test empty list against list with data are not the same
		assertFalse(UtilityFunctions.compareStringLists(list1, list2),
				"Expect an empty string list not to be considered identical to one containing data.");
		...
		// Test empty list against list with data are not the same
		assertFalse(UtilityFunctions.compareStringLists(list1, list2),
				"Expect an empty string list to not to be considered identical to one containing data.");

		list2.add("A String");

		// Test lists for different sizes are not the same
		assertFalse(UtilityFunctions.compareStringLists(list1, list2),
				"Expect string lists not to be considered identical when containing different numbers of items.");
		...
		assertTrue(UtilityFunctions.compareStringLists(list1, list2),
				"Expect string lists to be considered identical when containing the same items.");
		...
		// Test lists containing the same data are the same
		assertFalse(UtilityFunctions.compareStringLists(list1, list2),
				"Expect string lists not to be considered identical when containing the same items in a different order.");
	...
	public void testLengthUnitsConversions() {
		...
		assertEquals(metres, UtilityFunctions.lengthToMetres(feet, LengthUnit.Feet), 1e-6, "Converting feet to metres");
		assertEquals(feet, UtilityFunctions.lengthFromMetres(metres, LengthUnit.Feet), 1e-6,
				"Converting inclincation in degrees to percentage slope");

		double yards = 40;
		metres = 36.576;
		assertEquals(metres, UtilityFunctions.lengthToMetres(yards, LengthUnit.Yards), 1e-6,
				"Converting metres to yards");
		assertEquals(yards, UtilityFunctions.lengthFromMetres(metres, LengthUnit.Yards), 1e-6,
	 ...
	 * Tests conversion from percent to degrees and back. Tests conversion from
	 * grads to degrees and back.
	 */
	@Test
	public void testBearingToDegreesConversions() {
		...
		assertEquals(degrees, UtilityFunctions.bearingToDegrees(minutes, BearingUnit.Minutes), 1e-6,
				"Converting bearing in minutes to degrees");
		assertEquals(minutes, UtilityFunctions.bearingFromDegrees(degrees, BearingUnit.Minutes), 1e-6,
				"Converting bearing in degrees to minutes");

		double grads = 400;
		degrees = 360;
		assertEquals(degrees, UtilityFunctions.bearingToDegrees(grads, BearingUnit.Grads), 1e-6,
				"Converting bearing in grads to degrees");
		assertEquals(grads, UtilityFunctions.bearingFromDegrees(degrees, BearingUnit.Grads), 1e-6,
				"Converting bearing in degrees to grads");
	}
	...
	public void testGradientToDegreesConversions() {
		...
		assertEquals(degrees, UtilityFunctions.gradientToDegrees(percent, GradientUnit.Percent), 1e-6,
				"Converting percentage slope to inclincation degrees");
		assertEquals(percent, UtilityFunctions.gradientFromDegrees(degrees, GradientUnit.Percent), 1e-6,
				"Converting inclincation in degrees to percentage slope");
		...
		double grads = 400;
		degrees = 360;
		assertEquals(degrees, UtilityFunctions.gradientToDegrees(grads, GradientUnit.Grads), 1e-6,
				"Converting inclincation grads to degrees");
		assertEquals(grads, UtilityFunctions.gradientFromDegrees(degrees, GradientUnit.Grads), 1e-6,
				"Converting inclincation in degrees to grads");
