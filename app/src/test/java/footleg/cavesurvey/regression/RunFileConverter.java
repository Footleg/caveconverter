...
/**
 * Regression helper class which runs data file conversions and compares the
 ...
 * input file with
 * this prefix and the suffix '_in._extn_' and compare the output with a
 * reference file with
 * this prefix and the suffix '_ref._extn_' (where _extn_ is the appropriate
 * file extension for
 * the file format.
 * 
 * @author Footleg
...
public final class RunFileConverter {
	final static String refFilesLocation = "./src/test/resources/data/regression/";
	final static String genFilesLocation = "./build/generated/datafiles/";
	...
	public RunFileConverter() {
		new File(RunFileConverter.genFilesLocation).mkdirs(); // Create the output dir if it does not exist
	}
	...
	public static String convertPocketTopoToToporobot(String filePrefix, CmdlineSplaysOpt splaysOpt,
			CmdlineOpt genLRUDOpt,
			String charSetEncoding) throws ParseException, FileNotFoundException {
		// Compare resulting file with expected file
		String fileCompare = convertFile(filePrefix, SurveyDataInputFormats.PocketTopo,
				SurveyDataOutputFormats.Toporobot,
				splaysOpt, genLRUDOpt, charSetEncoding, new CmdLineLogger());
		return fileCompare;
	}
	...
	private static String generateFilename(String location, String filePrefix, SurveyDataInputFormats fromFormat,
			SurveyDataOutputFormats toFormat,
		...
		return location + filePrefix + "_" + letterCodeFromInputDataFormat(fromFormat)
				+ letterCodeFromOutputDataFormat(toFormat) + splayOptString + "_" + suffix
				+ generateFileSuffix(toFormat);
	}

	private static String generateRefFilename(String filePrefix, SurveyDataInputFormats fromFormat,
			SurveyDataOutputFormats toFormat, CmdlineSplaysOpt splaysOpt, boolean oldJRE) {
		String suffix = "ref";
		if (oldJRE) {
			suffix += "7";
		}
	...
	private static String generateOutputFilename(String filePrefix, SurveyDataInputFormats fromFormat,
			SurveyDataOutputFormats toFormat, CmdlineSplaysOpt splaysOpt) {
		return generateFilename(genFilesLocation, filePrefix, fromFormat, toFormat, splaysOpt, "testgen");
	}

	/**
	 * Sets the date used as 'today' in conversions so that output files match
	 * reference files
	 * with dates in them
	 */
	private static void setDateForConversions() {
		// Set date to a fixed date rather than the actual date at runtime so that test
		// output uses a known date
		try {
	...
	private static String convertFile(String filePrefix, SurveyDataInputFormats fromFormat,
			SurveyDataOutputFormats toFormat,
			CmdlineSplaysOpt splaysOpt, CmdlineOpt genLRUDOpt, String charSetEncoding, CmdLineLogger logger)
		...
		// Do file conversion
		CaveConverter.convertFile(inputFile, outputFile, fromFormat, toFormat, splaysOpt, genLRUDOpt, charSetEncoding,
				logger);
	...
	public static String convertUsingCmdLine(String filePrefix, char fromFormatCode, char toFormatCode, String options)
			throws ParseException, FileNotFoundException {
